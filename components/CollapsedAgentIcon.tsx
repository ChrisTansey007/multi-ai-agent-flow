...
import { IconCircuitBoard, IconCheck, IconAlertTriangle, IconSparkles } from './Icons';

const statusStyles: Record<NodeStatus, string> = {
    idle: 'border-[var(--agent-accent-color)]/50',
...
const StatusIcon = ({ status }: { status: NodeStatus }) => {
    switch(status) {
        ...
        default: return <IconCircuitBoard className="w-8 h-8 text-[var(--agent-accent-color)]" />;
    }
};

const CollapsedAgentIcon: React.FC<NodeProps<NodeData>> = ({ id, data }) => {
    ...
    return (
        <div
            ...
            className={`agent-icon w-16 h-16 rounded-full bg-black/60 backdrop-blur-md border-2 flex items-center justify-center cursor-move group nowheel transition-colors duration-300 ${borderClass}`}
            title={title}
            onDoubleClick={() => onToggleCollapse(id, 'agentPanel')}
        >
             <Handle type="target" position={Position.Left} className="!bg-cyan-500 !w-2 !h-2 !-ml-1" />
            
            <div
                className="w-12 h-12 rounded-full bg-[var(--agent-accent-color)]/20 flex items-center justify-center"
            >
...
export default memo(CollapsedAgentIcon);